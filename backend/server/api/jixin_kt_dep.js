...
const KNOWLEDGETAGGING_SCRIPT_PATH = process.env.KNOWLEDGETAGGING_SCRIPT_PATH || '/root/Jixin_kt_deploy/deploy_0412.py';

let kt = (req, res) => {
    console.log('knowledgetagging_jixin');
    let db = couchdb.use('roy-config');
    let attachs = [], new_files = [], check = 0, readCheck = 0, outputs = {}, received_files = [];
    let default_folder = 'kt_jixin_image_' + req.headers['verified-token'].email + '-' + new Date().getTime(),
        dir_path = '/tmp/',
        file_name = 'output.json',
        cb_url = req.headers.origin || 'localhost:2978',
        api_path = req.body.cb_api || '/api/callbackktjx',
        uuid = req.body.uuid || '';
    // cb_url = path.join(cb_url, api_path);
    res.status(200).send('File upload is done, prepare tagging knowledge');
    res.end();
    ...
    fs.rename(file.path, dir_path + default_folder + '/' + file.originalFilename, (err) => {
        ...
        let execPythonScript = spawn('python3.6', [KNOWLEDGETAGGING_SCRIPT_PATH, '-i', dir_path + default_folder + '/' + file.originalFilename, '-o', dir_path + default_folder + '/' + file_name]);
        ...
        execPythonScript.on('close', (code) => {
            ...
            let output = "This server currently does not support knowledge tagging(jixin version) function page. Please contact with us";
            try {
                ...
                new_files.forEach(new_file => {
                    ...
                    fs.readFile(new_file.path, (err, data) => {
                        ...
                        if (--readCheck === 0) {
                            ...
                            //
                            var doc_name = 'KT-JX-(' + req.headers['verified-token'].email + ')-' + new Date().getTime();
                            db.multipart.insert({ email: req.headers['verified-token'].email, output: output, output_dir: default_folder, created_data: new Date().getTime(), doc_type: 'ktjx' }, attachs, doc_name, (err, body) => {
                                if (err) {
                                    console.log(err);
                                }
                                return;
